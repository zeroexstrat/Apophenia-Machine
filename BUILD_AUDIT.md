...
|-------|----------|-----------|--------------|
| `ingest.py` | `vigil start` | Corpus checks processed library schema and claim/evidence trace presence | Ignored runtime state refresh |
| `exhaust.py` | `vigil start` plus cursor guard | Calcinatio checks schema/trace/ceiling; Caput Mortuum checks exact ID/depth state | Ignored runtime state refresh |
| `connect.py` | `vigil start` | Coniunctio checks connection schema, records, evidence fields, and declared citations | Ignored runtime state refresh |
| `detect.py` | `vigil start` | Nigredo Redux checks durable Rubedo rejection fingerprints | Ignored runtime state refresh |
| `draft.py` | `vigil start` | `vigil verify` runs the bounded structural gate set | Ignored runtime state refresh |

These checks do not establish scientific truth, logical entailment, evidence adequacy, or literature-wide novelty.
