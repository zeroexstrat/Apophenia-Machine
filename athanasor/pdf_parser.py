...
def parse_pdf(path: str | Path) -> dict[str, Any]:
    ...
    if fitz is None:
        ...
        return {
            ...
            "page_count": max(1, fallback_text.count("\f") + 1),
            "full_text": full_text,
...
def parse_text_file(path: str | Path) -> dict[str, Any]:
    """Parse a plain-text or Markdown document into the parse_pdf record shape."""
    text_path = Path(path).expanduser().resolve()
    if not text_path.exists():
        raise FileNotFoundError(f"Text document not found: {text_path}")
    ...
    for line in lines[:60]:
        normalized = line.strip()
        if len(normalized) > 30 and _looks_like_abstract(normalized):
            abstract = normalized
            break
    ...
    return {
        ...
        "full_text": full_text,
        "sections": _merge_sections(_detect_section_headers(lines, 1)),
        "references": [],
        "equations": _extract_equation_lines(full_text),
        "abstract": abstract,
        "parse_warnings": [],
        "encrypted": False,
        "image_only": len(full_text) == 0,
    }
