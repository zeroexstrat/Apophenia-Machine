...
class Registry:
    ...
    def _write_all(self, entries: list[dict[str, Any]]) -> None:
        # Serialize first, then swap atomically: a crash mid-write must never
        # truncate or corrupt the canonical registry file.
        rendered = "".join(json.dumps(entry, sort_keys=False) + "\n" for entry in entries)
