...
def _sanitize_sections(merged: dict[str, Any]) -> dict[str, Any]:
    """Restore defaults for sections that are null or the wrong shape.

    A hand-edited config with an empty section (``llm:`` with nothing after it)
    parses as ``None`` and must not crash config loading.
    """
    defaults = _default_config()
    for section in ("llm", "embeddings", "paths", "exhaustion"):
        if not isinstance(merged.get(section), dict):
            merged[section] = defaults[section]
    domains = merged.get("domains")
    if not isinstance(domains, list) or not domains:
        merged["domains"] = defaults["domains"]
    return merged
