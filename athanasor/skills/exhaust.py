...
def _process_one(
    *,
    ...
    claims = _dict_items(record.get("claims"))
    methods = _dict_items(record.get("methods"))
    techniques = _dict_items(record.get("techniques"))
    equations = _dict_items(record.get("equations"))
    ...
    # Guardrail windows must cover the configured thresholds, or a threshold
    # above the window size could never fire.
    redundancy_window = max(5, redundancy_stop_threshold)
    speculative_window = max(1, speculative_stop_count)
    recent_redundant = deque(maxlen=redundancy_window)
    recent_speculative = deque(maxlen=speculative_window)
    ...
    while _non_redundant_count(all_items) < max_items and iteration_guard < 100:
        ...
        # Only stop for lack of progress after ALL buckets in the batch were
        # consumed; breaking inside the bucket loop would discard buckets that
        # follow an empty one.
        ...
        if len(recent_redundant) >= redundancy_window and sum(recent_redundant) >= redundancy_stop_threshold:
            terminal_reason = "redundancy"
            break

        if len(recent_speculative) >= speculative_window and sum(recent_speculative) == len(recent_speculative):
...
def _dict_items(value: Any) -> list[dict[str, Any]]:
    """Keep only mapping entries from a possibly hand-edited YAML list."""
...
def _item_key(bucket: str, item: dict[str, Any]) -> str:
    ...
    # which would mint a new embedding key for identical text on every rerun.
...
# Primary text field per bucket, used to wrap bare-string items from a model.
_BUCKET_PRIMARY = {
    "derivations": "statement",
    "exercises": "problem",
    "missing_angles": "angle",
    "open_questions": "question",
    "unstated_assumptions": "assumption",
    "experiments": "hypothesis",
    "necessary_connections": "work",
}


def apply_agent_exhaust(
    records: list[dict[str, Any]],
    *,
    config: Config | None = None,
    registry: Registry | None = None,
) -> list[dict[str, Any]]:
    """File exhaustion records produced by the *driving agent* (no LLM backend).

    Each record names a paper and supplies bucket items. Items are run through the
    same ``_normalize_item`` alias tolerance and ``EXHAUST_SCHEMA`` validation as the
    backend path, so agent output from any model files identically and comparably.
    """
    config = config or load_config()
    root = Path(config.project_root).expanduser().resolve()
    paths = config.resolved_paths
    registry = registry or Registry(root / "albedo" / "registry.jsonl")
    schema = _load_schema()
    library_root = paths["albedo"] / "library"
    exhaust_root = paths["albedo"] / "exhaust"
    ...
    for record in records:
        if not isinstance(record, dict):
            results.append({"paper_id": None, "action": "invalid_record"})
            continue
        paper_id = str(record.get("paper_id") or "").strip()
        entry = registry.get(paper_id) if paper_id else None
        if entry is None:
            results.append({"paper_id": paper_id or None, "action": "not_found"})
            continue

        buckets = _collect_agent_buckets(record)
        item_total = sum(len(items) for items in buckets.values())
        if item_total == 0:
            results.append({"paper_id": paper_id, "action": "empty"})
            continue

        depth = _coerce_depth(record.get("depth") or record.get("exhaustion_depth"))
        library_record = _load_library_for_exhaust(library_root / f"{paper_id}.yaml")
        title = str((library_record.get("source") or {}).get("title") or entry.get("title") or paper_id)
        domain = str(entry.get("domain") or "unclassified")
        paper_type = _infer_paper_type(
            library_record.get("tags", []),
            library_record.get("methods", []),
            library_record.get("techniques", []),
        )
        ...
        payload: dict[str, Any] = {
            ...
            "exhaustion": {
                ...
                "exhaustion_depth": depth,
                "schema_version": 2,
                "agent": "azoth-agent-exhaust",
                "date": now_iso(),
                "termination": {
                    "criterion": "completed",
                    "detail": f"agent_supplied; items={item_total}",
                    "deeper_available": depth < 5,
                },
            },
        }
        ...
        registry.update(
            ...
            {
                "status": "exhausted",
                "exhausted_at_depth": max(int(entry.get("exhausted_at_depth") or 0), depth),
                "paths": {
                    **(entry.get("paths") or {}),
                    "exhaust": str(out_path.relative_to(root)),
                },
            },
        )
        results.append(
            {
                ...
                "depth": depth,
                "source": "agent",
                "counts": {name: len(items) for name, items in buckets.items() if items},
            }
        )
...
def _collect_agent_buckets(record: dict[str, Any]) -> dict[str, list[dict[str, Any]]]:
    buckets: dict[str, list[dict[str, Any]]] = {name: [] for name in BUCKET_NAMES}
    for name in BUCKET_NAMES:
        raw_items = record.get(name)
        if not isinstance(raw_items, list):
            continue
        for raw in raw_items:
            if isinstance(raw, str):
                raw = {_BUCKET_PRIMARY[name]: raw}
            if not isinstance(raw, dict):
                continue
            payload = _normalize_item(name, raw)
            if not _item_text(name, payload):
                continue
            payload["redundant"] = False
            payload["source_claim"] = _coerce_source_claim(payload.get("source_claim"))
...
def _load_library_for_exhaust(path: Path) -> dict[str, Any]:
    with_record = {}
    try:
        if path.exists():
