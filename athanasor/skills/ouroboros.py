...
def _assert_public_host(url: str) -> None:
    """Reject URLs whose host resolves to a private/loopback/internal address.

    Prior-art source lists are human-curated, but a mistaken or malicious URL must
    not turn the downloader into a probe of the local network.
    """
    host = urlparse(url).hostname
    if not host:
        raise RuntimeError(f"URL has no resolvable host: {url}")
    try:
    ...
    for info in infos:
        ...
        if (
            ...
            raise RuntimeError(
                f"Host {host!r} resolves to a private/internal address ({raw}); refusing to fetch."
            )
...
def _download_pdf(url: str, target_path: Path, timeout: float = 30) -> DownloadResult:
    ...
    try:
        with urllib.request.urlopen(request, timeout=timeout) as response:
            ...
            # Bound the read so a misbehaving source cannot exhaust memory/disk.
            data = response.read(MAX_DOWNLOAD_BYTES + 1)
    except urllib.error.URLError as exc:
        raise RuntimeError(str(exc)) from exc

    if final_url != url:
        # Redirects are followed by urllib; re-validate wherever we actually landed.
        parsed = urlparse(final_url)
        if parsed.scheme not in {"http", "https"}:
            raise RuntimeError(f"Redirected to non-http(s) URL: {final_url}")
        _assert_public_host(final_url)

    if len(data) > MAX_DOWNLOAD_BYTES:
        raise RuntimeError(
            f"Download exceeds size cap of {MAX_DOWNLOAD_BYTES} bytes; fetch manually if intended."
        )

    # Trust content, not filenames: the payload must actually be a PDF.
    if not data.startswith(b"%PDF"):
        raise RuntimeError(
            f"Response is not a PDF (magic bytes absent; content-type={content_type or 'unknown'})."
        )
