...
def _skip_record(path: Path, reason: str) -> dict[str, Any]:
    return {
        "paper_id": None,
        "filename": path.name,
        "status": "skipped",
...
def ingest_path(
    ...
    *,
    ...
    # Content hashes already in the registry (older entries may predate hashing).
    known_hashes: dict[str, str] = {}
    for existing in registry.list():
        existing_hash = existing.get("content_hash")
        if isinstance(existing_hash, str) and existing_hash:
            known_hashes[existing_hash] = str(existing.get("paper_id") or "")

    for file_path in files:
        try:
            parsed = _parse_document(file_path)
        except Exception as exc:
            outputs.append(_skip_record(file_path, f"parse failed: {exc}"))
            continue
        if parsed.get("encrypted"):
            outputs.append(_skip_record(file_path, "PDF is encrypted; manual handling required."))
            continue
        if parsed.get("image_only"):
            outputs.append(
                _skip_record(file_path, "No extractable text (image-only or empty document).")
            )
            continue

        content_hash = _file_sha1(file_path)
        if not reprocess and content_hash in known_hashes:
            duplicate_of = known_hashes[content_hash]
            stored = common.move_to_domain(
                file_path, nigredo_dir / "duplicates", filename=file_path.name
            )
            record = _skip_record(
                file_path,
                f"duplicate content of {duplicate_of or 'an existing paper'}; "
                f"moved to nigredo/duplicates/{stored.name}",
            )
            record["duplicate_of"] = duplicate_of or None
        ...
        allowed_domains = set(config.domains) | {"unclassified"}
        if domain not in allowed_domains:
            domain = "unclassified"

        domain_dir = nigredo_dir / domain
        if file_path.parent.resolve() == domain_dir.resolve():
            # Reprocessing in place: moving would rename the file and mint a new id.
            destination_pdf = file_path
        else:
        ...
        payload["id"] = paper_id
        # An LLM payload may omit `source` or return it with the wrong shape.
        if not isinstance(payload.get("source"), dict):
            payload["source"] = {}
        payload["source"].setdefault("authors", authors or ["Unknown"])
        ...
        if registry.exists(paper_id):
            fields = dict(entry)
            existing_entry = registry.get(paper_id) or {}
            if existing_entry.get("status") == "exhausted":
                # Never downgrade pipeline status during a reprocess ingest.
                fields.pop("status", None)
            registry.update(paper_id, fields)
        else:
            registry.add(entry)
        known_hashes[content_hash] = paper_id
...
def _gather_files(target: Path) -> list[Path]:
    suffixes = (".pdf", *sorted(TEXT_SUFFIXES))
    if target.is_file():
