...
from ..skills.common import load_yaml_tolerant, now_iso, run_vigil_check
    ...
    *,
    ...
    if gap_id:
        ...
        if not explicit.exists():
            raise FileNotFoundError(f"Hypothesis not found: {explicit}")
        candidates = [explicit]
    else:
...
def _frontmatter(gap_id: str, papers: list[str], title: str) -> str:
    ...
    # titles containing colons, quotes, or `#` cannot break the frontmatter.
    safe_title = json.dumps(" ".join(str(title).split()))
    safe_papers = ", ".join(json.dumps(str(paper)) for paper in papers)
    return (
        "---\n"
        f"gap_id: {json.dumps(str(gap_id))}\n"
        f"title: {safe_title}\n"
        f"date: {date}\n"
        "status: pending_review\n"
        f"papers: [{safe_papers}]\n"
        "---\n\n"
    )
