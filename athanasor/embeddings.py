    ...
    def __init__(self, path: str | Path, model_name: str = "all-MiniLM-L6-v2"):
        ...
        if SentenceTransformer is not None:
            try:
                ...
                probe = getattr(self._model, "get_embedding_dimension", None) or getattr(
                    self._model, "get_sentence_embedding_dimension"
                )
                self._dimension = int(probe())
            except Exception:
                self._model = None
                self._dimension = 384
        self._ids: list[str] = []
    ...
    def load(self) -> None:
        ...
        try:
            ...
            if matrix.ndim != 2 or matrix.shape[0] != len(ids):
                # A partial save left ids and vectors misaligned; serving them
                # would return wrong-id results forever. Reset instead.
                raise ValueError("embedding store metadata/vector mismatch")
            self._ids = list(ids)
